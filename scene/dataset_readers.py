...
def readWaymoInfo(path, white_background, eval, extension=".png", use_bg_gs=False, 
    ...
    for t in range(start_time, end_time):
        ...
        # lidar to world : lidar = ego in waymo/carla
...
def readCarlaInfo(path, white_background, eval, extension=".png", use_bg_gs=False, 
                  load_sky_mask = False, load_panoptic_mask = True, load_sam_mask = False,load_dynamic_mask = False,
                  load_feat_map = False,
                  load_intrinsic = False, load_c2w = False,
                  start_time = 0, end_time = -1, num_pts = 5000, 
    ...
    print("use_bg_gs: ", use_bg_gs)
    print("load_sky_mask: ", load_sky_mask)
    print("load_panoptic_mask: ", load_panoptic_mask)
    print("load_sam_mask: ", load_sam_mask)
    print("load_dynamic_mask: ", load_dynamic_mask)  # True
    print("load_feat_map: ", load_feat_map) # True
    print("load_intrinsic: ", load_intrinsic)
    print("load_c2w: ", load_c2w)
    print("start_time: ", start_time)
    print("end_time: ", end_time)
    print("num_pts: ", num_pts)
    print("save_occ_grid: ", save_occ_grid) # True
    print("occ_voxel_size: ", occ_voxel_size)
    print("recompute_occ_grid: ", recompute_occ_grid)
    print("stride: ", stride)
    print("original_start_time: ", original_start_time)
    print("=================================================")

    load_dynamic_mask = False
    print("============ load_dynamic_mask manually set to False ============")

    ORIGINAL_SIZE = [[1280, 1920], [1280, 1920], [1280, 1920], [1280, 1920], [1280, 1920]]
    # opencv coordinate system: x right, y down, z front
    # carla coordinate system: x front, y right, z up
    # opencv_cam -> carla_cam
    OPENCV2DATASET = np.array(
        [[0, 0, 1, 0], 
         [-1, 0, 0, 0], 
         [0, -1, 0, 0], 
    ...
    num_seqs = len(os.listdir(image_folder))/5
    start_time = start_time
    print("======= start_time is set to ", start_time)
    print("======= end time is set to ", end_time)
    if end_time == -1: # default to the end of the sequence
        end_time = int(num_seqs)
        print("======= end_time is set to the end of the sequence, end index = ", end_time)
    else: # end_time is the last frame index
        end_time += 1
        print("======= end_time is set to the last frame index, end index = ", end_time)

    camera_list = [1,0,2] # only use the front_left, left, front_right cameras

    # set lidar range trucated range
    truncated_min_range, truncated_max_range = -2, 80

    # set camera frustum range
    cam_frustum_range = [0.01, 80]
    ...
    # reading image paths
    print("Reading image & lidar paths")
    for t in range(start_time, end_time):
        for cam_idx in camera_list:
            img_filepaths.append(os.path.join(data_root, "images", f"{t:03d}_{cam_idx}.jpg"))
            #dynamic_mask_filepaths.append(os.path.join(data_root, "dynamic_masks", f"{t:03d}_{cam_idx}.png"))
    ...
    if load_feat_map:
        print("load_feat_map is true, extract features")
        return_dict = extract_and_save_features(
                input_img_path_list=img_filepaths,
                saved_feat_path_list=feat_map_filepaths,
                img_shape=[644, 966],
    ...
    # load poses: intrinsic, c2w, l2w
    # ------------------
    print("Loading poses: intrinsis, extrinsics, c2w, l2w")
    _intrinsics = []
    cam_to_egos = []
    for i in range(len(camera_list)):
        # load intrinsics
        ...
        # opencv coordinate system: x right, y down, z front
        # carla coordinate system: x front, y right, z up
        cam_to_egos.append(cam_to_ego @ OPENCV2DATASET) # opencv_cam -> carla_cam -> carla_ego

    # compute per-image poses and intrinsics
    print("computing per-image poses and intrinsics")
    cam_to_worlds, ego_to_worlds = [], []
    intrinsics, cam_ids = [], []
    lidar_to_worlds = []

    # ===! for carla, we simplify timestamps as the time indices
    timestamps, timesteps = [], []
    # we tranform the camera poses w.r.t. the first timestep to make the translation vector of
    # the first ego pose as the origin of the world coordinate system.
    ego_to_worlds_start = np.loadtxt(os.path.join(data_root, "ego_pose", f"{start_time:03d}.txt"))
    for t in range(start_time, end_time):
        ego_to_world_current = np.loadtxt(os.path.join(data_root, "ego_pose", f"{t:03d}.txt"))
        # ego to world transformation: cur_ego -> world -> start_ego(world)
        ego_to_world = np.linalg.inv(ego_to_worlds_start) @ ego_to_world_current
        ego_to_worlds.append(ego_to_world)
        for cam_id in camera_list:
            cam_ids.append(cam_id)
            # transformation:
            # opencv_cam -> carla_cam -> carla_cur_ego -> world -> start_ego(world)
            cam2world = ego_to_world @ cam_to_egos[cam_id]
            cam_to_worlds.append(cam2world)
            intrinsics.append(_intrinsics[cam_id])
            # ===! we use time indices as the timestamp for carla dataset for simplicity
            # ===! we can use the actual timestamps if needed
            # to be improved
            timestamps.append(t - start_time)
            timesteps.append(t - start_time)
        # lidar to world : lidar = ego in carla
        lidar_to_worlds.append(ego_to_world)
    ...
    # ------------------
    # compute frustums
    print("computing camera frustum aabb")
    frustums = []
    pix_corners = np.array( # load_size : [h, w]
        [[0,0],[0,load_size[0]],[load_size[1],load_size[0]],[load_size[1],0]]
    )
    ...
    # ------------------
    # mask
    print("splitting train and test from timestamps")
    if stride != 0 :
        train_mask = (timestamps % int(stride) != 0) | (timestamps == 0)
    else:
        train_mask = np.ones(len(timestamps), dtype=bool)
    ...
    # load points and depth map
    # ------------------
    print("loading points and depth map")
    pts_path = os.path.join(data_root, "lidar")
    load_lidar, load_depthmap = True, True
    depth_maps = None
    # bg-gs settings
    ...
    if not os.path.exists(pts_path) or not load_lidar:
        ...
        shs = np.random.random((num_pts, 3)) / 255.0
        pcd = BasicPointCloud(points=xyz, colors=SH2RGB(shs), normals=np.zeros((num_pts, 3)))
        print("done generating random point cloud")
    # storePly(ply_path, xyz, SH2RGB(shs) * 255)
    else:
        # load lidar points
        print("========= find lidar data ==========")
        ...
        for t in trange(0, len(lidar_filepaths), desc="loading lidar", dynamic_ncols=True):
            ...
            lidar_origins = lidar_info[:, :3] # origin_x, origin_y, origin_z
            lidar_points = lidar_info[:, 3:6] # point_x, point_y, point_z
            # lidar_ids = lidar_info[:, -1] # laser_id
            # lidar_ids = 0
            # select lidar points based on a truncated ego-forward-directional range
            # make sure most of lidar points are within the range of the camera
            valid_mask = lidar_points[:, 0] < truncated_max_range
        ...
        ply_path = os.path.join(data_root, "ds-points3d.ply")
        storePly(ply_path, points, SH2RGB(shs) * 255)
        print("store lidar initial ply to ", ply_path)
        pcd = BasicPointCloud(points=points, colors=SH2RGB(shs), normals=np.zeros((len(points), 3)))  
        if use_bg_gs:
            bg_ply_path = os.path.join(data_root, "ds-bg-points3d.ply")
            storePly(bg_ply_path, bg_points, SH2RGB(bg_shs) * 255)
        ...
        print("======== Done loading lidar points ========")

    # ------------------
    # prepare cam-pose dict
    # ------------------
    print("Preparing cam-pose dict")
