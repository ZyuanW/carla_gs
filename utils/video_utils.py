...
def render_func(
    ...
    with torch.no_grad():
        for i in tqdm(range(len(viewpoint_stack)), desc=f"rendering full data", dynamic_ncols=True):
            ...
            if compute_metrics:
                ...
                if hasattr(viewpoint_cam, 'dynamic_mask') and viewpoint_cam.dynamic_mask is not None:
                    dynamic_mask = get_numpy(viewpoint_cam.dynamic_mask).astype(bool)
                    print("dynamic_mask.sum :", dynamic_mask.sum())
                    if dynamic_mask.sum() > 0:
                        print("dynamic_mask.sum > 0 -->:", dynamic_mask.sum())
                    ...
                    print("No dynamic mask found; skipping dynamic mask evaluation.")
    ...
    if len(dynamic_rgbs)>0:
        print("len dynamic_rgbs > 0 :",len(dynamic_rgbs))
...
def save_videos(
    ...
    print("save video - keys:",keys) 
    if save_seperate_video:
        print("save_seperate_video")
...
def save_concatenated_videos(
    ...
    print("save_concatenated_videos -- keys:",keys)
...
def save_seperate_videos(
    ...
    print("save_seperate_videos -- keys:",keys)
    return_frame_id = num_timestamps // 2
    return_frame_dict = {}
    for key in keys:
